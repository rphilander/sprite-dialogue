...
- **From Claude** → conversational text is automatically mirrored to the UI (no tool calls needed); images go via a `send_image` tool
...
- **Outbound text (Claude → you)**: the bun MCP server tails `~/.claude/projects/*/*.jsonl`, parses new assistant turns, and broadcasts them to the UI WebSocket. Race-free by construction — we read after Claude Code has already flushed.
- **Outbound images (Claude → you)**: `send_image` tool with absolute file path
- **Terminal user input**: the same transcript watcher picks up `user`-type entries (skipping channel-sourced messages and tool results) so the UI shows what was typed in the terminal too
